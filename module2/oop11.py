...
# its a custom decorator used to define class property similer to @property for instance variables
...
class LuxuryWatch(Watch):
    ...
    # a class method to access class variable
    @classmethod
    def get_watch_count(cls):
        return cls.__watches_created

    # using @classproperty decorator for the same perpose to get class variable
    # just that it acts a property now (class property)
    @classproperty
    def watches_created(cls):
        return cls.__watches_created

    # allowing to create object with extra params
    # with extra parmams
    @classmethod
    def with_engraving(cls, engrave):
        _cls = cls()
        _cls.engrave = engrave
        return _cls
